...
BASE_DIR="/leonardo_scratch/fast/L-AUT_Coretti/CriticalNucleus/Crystalline"

# Define the Python script path
PYTHON_SCRIPT="./write_mcg.py"
...
for parent_folder in "$BASE_DIR"/*; do
	...
	folder_name=$(basename "$parent_folder")
        ...
        if [[ "$folder_number" -gt 100 && "$folder_number" -lt 200 ]]; then
		...
		for subfolder in "$parent_folder"/*; do
			...
			for xtc_file in "$subfolder"/*.h5; do
