...
optimal_mcg_values = np.array([165, 170, 173, 175, 177, 180, 185])

base_input_path = '/leonardo_work/L-AUT_Coretti/CODE/ClathrateTPS'
traj_list_file = 'AmorphousTrajectoriesList.txt'
...
folder = "$FAST/CriticalNucleus/Amorphous"
...
for optimal_mcg in optimal_mcg_values:#closest_indices:
    ...
    for sim_num in range(len(random_numbers)):
        ...
        job_name = f"a_sim_{mcg_value}_{sim_num}"
        subprocess.run([
            "sbatch",
            "--job-name", job_name,
