...
/**
 * @brief Calculates the pairwise distances between solute and solvent atoms within a given threshold and updates the neighboring lists.
 *
 * This function computes the distances between each pair of solvent atoms, and between solute and solvent atoms. It updates the `My_neigh` vector with the neighboring indices of each atom, and the `Nneigh` vector with the number of neighbors for each atom. It also handles periodic boundary conditions for atom positions in a 3D box.
 * 
 * The function assumes that solvent atoms are represented every 4th index in the `atom_Pos` array (with indices starting after the solute atoms), and it considers the oxygen atom as the reference atom for the solvent.
 * 
 * The periodic boundary conditions are applied by checking the distance in each dimension (x, y, z) and adjusting if the distance exceeds half of the box size.
 * 
 * The function only considers pairs of atoms where the distance between them is greater than 0.18 and less than or equal to the defined `HBOND_DIST` threshold.
 *
 * @param count_solvent The number of solvent molecules to process.
 * @param count_solute The number of solute molecules.
 * @param My_neigh A reference to a 2D vector of integers that will be updated with the indices of the neighboring atoms for each atom.
 * @param atom_Pos A reference to a 2D vector of doubles, where each row contains the (x, y, z) coordinates of each atom.
 * @param boxX The length of the simulation box in the X dimension.
 * @param boxY The length of the simulation box in the Y dimension.
 * @param boxZ The length of the simulation box in the Z dimension.
 * @param Nneigh A reference to a vector of integers, which will be updated with the number of neighbors for each atom.
 * @param Natoms The total number of atoms in the system.
 * @param topSolute The index of the last solute atom in the list (atoms after this index are solvent).
 * @param time A string representing the current time or time step in the simulation.
 * @param HBOND_DIST The maximum distance (in Angstroms) for considering an atom pair as neighbors (for example, for hydrogen bonding calculations).
 */
...
void calc_Distance(int count_solvent, int count_solute, vector<vector<int>>& My_neigh, vector<vector<double>>& atom_Pos, double& boxX, double& boxY, double& boxZ, vector<int>& Nneigh, int Natoms, int topSolute, string time, double HBOND_DIST ){
    ...
    for (int i = topSolute+1 ; i < topSolute + count_solvent ; i+=4)      // i is Oxygen index
    {
        ...
        // std::cout << i/4 << endl;
        // for (int k = 0; k < temp_vect2.size(); k++) {
        //     cout << std::setprecision(3);
        //     std::cout<< temp_vect2[k]/4  << endl;
        //     std::cout<< atom_Pos[temp_vect2[k]][0] << endl;
        // }
        
        My_neigh.push_back(temp_vect2);
        Nneigh.push_back(neigh_counter);
        
        My_neigh.push_back({0});            // to keep indices consistend with atom_pos (since only every fourth is an oxygen)
...
void ring_Finder(int count_solute, int Natoms, vector<int>& Nneigh, vector<vector<int>>& My_neigh, vector<vector<int>>& ring4_temp, vector<vector<int>>& ring5_temp, vector<vector<int>>& ring6_temp, int topSolute, int count_solvent, vector<vector<double>>& atom_Pos, double boxX, double boxY, double boxZ, double HBOND_DIST, double delta_s, double delta_p, double delta_h){
    ...
    int k4=0, k5=0, k6=0;
    vector<int> temp4, temp5, temp6 ;
    vector<vector<int>> m4, m5, m6;
    m4.clear();
    m5.clear();
    m6.clear();
    vector<int> temp_vec4={0,0,0,0,0}, temp_vec5={0,0,0,0,0,0}, temp_vec6={0,0,0,0,0,0,0}, temp_vecn={0};
    vector<vector<int>> My_neigh_ring6, My_neigh_ring5, My_neigh_ring6_ring5;
    bool isPresent = false;
    double dist_a1_a4 = 0, dist_a1_a3 = 0 , dist_a2_a4, dist_a2_a5 = 0, dist_a3_a6 = 0;
    double dx,dy,dz;

    double Lrange4 = 0;// 1.4 * HBOND_DIST - delta_s;
    //double delta1_p = 0.18;
    double Lrange5 = 1.6 * HBOND_DIST - delta_p;            //londest diagonal in a pentagon is from 1 to 3/4 with a length of 1.618
    ...
    for( a_1 = topSolute+1; a_1 < (topSolute + count_solvent) ; a_1+=4)   // loops over all waters (specifically the oxygen of water)
    {
        for(int m_1 = 1 ; m_1 <= Nneigh[(a_1 -topSolute+1)/4] ; m_1++)  // loops over all indices of neighbours of a1
        {
            a_2 = My_neigh[a_1][m_1];                                 // a_2 is now the neighbour index (as compatible with atom_Pos)
            if (a_2 != a_1)                                          // can't be the same as the first atom
            {
                for(int m_2 = 1 ; m_2 <= Nneigh[(a_2 -topSolute+1)/4] ; m_2++) // loops over all indices of neighbours of a2
                {
                    a_3 = My_neigh[a_2][m_2];
                    
                    if(a_3 != a_1)                                  // why do we not check whether or not a3 is a2
                    {
                        for(int m_3 = 1 ; m_3 <= Nneigh[(a_3 -topSolute+1)/4] ; m_3++)
                        {
                            ...
                            if(a_4 != a_2 && a_4 != a_1)
                            {
                                ...
                                {
                                    for(q_1 = 1 ; q_1 <= Nneigh[(a_1 -topSolute+1)/4] ; q_1++)
                                    {
                                        if( a_4 == My_neigh[a_1][q_1-1])
                                        {
                                            ...
                                            if( dist_a1_a3 > Lrange4  )
                                            {
                                                dx = atom_Pos[a_2][0] - atom_Pos[a_4][0];
                                                dy = atom_Pos[a_2][1] - atom_Pos[a_4][1];
                                                dz = atom_Pos[a_2][2] - atom_Pos[a_4][2];
                                                
                                                if (abs(dx) >= boxX * 0.5){ dx = boxX - abs(dx) ;}
                                                if (abs(dy) >= boxY * 0.5){ dy = boxY - abs(dy) ;}
                                                if (abs(dz) >= boxZ * 0.5){ dz = boxZ - abs(dz) ;}
                                                
                                                dist_a2_a4 = sqrt(dx*dx + dy*dy + dz*dz);
                                                //if (dist_a1_a3 > 0.37 && dist_a1_a3 < 0.53 )
                                                if (dist_a2_a4 > Lrange4 )
                                                {
                                            ...
                                            //*************
                                                    ...
                                                    if (k4==0) //put the first ring into matrix m4.
                                                    {
                                                        m4.push_back(temp4);
                                                        k4++;
                                                        ring4_temp.push_back(temp_vec4);
                                                    }

                                                    // std::cout<<"m4 size after if " << m4.size() << endl;

                                                    // for (const auto& vec : ring4_temp) {
                                                    //     for (int num : vec) {
                                                    //         std::cout << num << " ";
                                                    //     }
                                                    ...
                                                    {  
                                                        ...
                                                        if(m4[i][0] == temp4[0] && m4[i][1] == temp4[1] && m4[i][2] == temp4[2] && m4[i][3] == temp4[3])  // TODO there is some sort of mistake here ??
                                                        {
                                                            isPresent=true ;
                                                            break;
                                                        }
                                                        else isPresent=false;       //this checks only this specific order
                                                    }
                                    ...
                                    a_5 = My_neigh[a_4][m_4];                           // TODO this line keeps making problems
...
void find_shared_edges_ring6_ring5(int count_ring6, int count_ring5, vector<vector<int>>& ring5, vector<vector<int>>& ring6, vector<vector<int>>& My_neigh_ring6_ring5, vector<unsigned long int>& N_ring6_ring5_neigh ){
    ...
    for (int ring6_idx = 0 ; ring6_idx < count_ring6 ; ring6_idx++)
    {
        for (int ring6_node = 0 ; ring6_node < ring6[ring6_idx].size() ; ring6_node++)
        {
            for (int ring5_idx = 0 ; ring5_idx < count_ring5 ; ring5_idx++)
            {
                for (int ring5_node = 0 ; ring5_node < ring5[ring5_idx].size() ; ring5_node++)
                {
                    // old: if ( ring6_idx != ring5_idx && ring6_node+1 < ring6[ring6_idx].size() && ( (ring6[ring6_idx][ring6_node] == ring5[ring5_idx][ring5_node] && ring6[ring6_idx][ring6_node+1] == ring5[ring5_idx][ring5_node+1]) || (ring6[ring6_idx][ring6_node] == ring5[ring5_idx][ring5_node+1] && ring6[ring6_idx][ring6_node+1] == ring5[ring5_idx][ring5_node]) )  )
                    if ( ring6_node+1 < ring6[ring6_idx].size() && ( (ring6[ring6_idx][ring6_node] == ring5[ring5_idx][ring5_node] && ring6[ring6_idx][ring6_node+1] == ring5[ring5_idx][ring5_node+1]) || (ring6[ring6_idx][ring6_node] == ring5[ring5_idx][ring5_node+1] && ring6[ring6_idx][ring6_node+1] == ring5[ring5_idx][ring5_node]) )  )
                    // i think the ring6_idx != ring5_idx was at fault for not finding pentagon 0 as a neighbour for hexagon 0, what sense does it make anyways?
                    // is there a point to the ring6_node+1 < ring6[ring6_idx].size() and not initialising the loop
                    {
                        counter++;
                        neigh_ring_counter++;
                        temp_vec4[0] = ring6_idx+1;
                        temp_vec4[1] = ring5_idx+1;
                        temp_vec4[2] = ring6[ring6_idx][ring6_node];
                        temp_vec4[3] = ring6[ring6_idx][ring6_node+1];
                        
                        ring6_ring5_neigh.push_back(temp_vec4);
                        
                        temp_vecn.push_back(ring5_idx);
    ...
    for (int ring6_idx = 0 ; ring6_idx < count_ring6 ; ring6_idx++)
    {
        N_ring6_ring5_neigh.push_back(My_neigh_ring6_ring5[ring6_idx].size()) ;
    }
...
void find_shared_edges_ring5_ring6(int count_ring5, int count_ring6, vector<vector<int>>& ring6, vector<vector<int>>& ring5, vector<vector<int>>& My_neigh_ring5_ring6, vector<unsigned long int>& N_ring5_ring6_neigh ){
    ...
    int counter=0;
    int neigh_ring_counter=0;
    //vector<int> N_ring5_ring6_neigh;
    vector<vector<int>> ring5_ring6_neigh;
    vector<int> temp_vec4 = {0,0,0,0};
    ...
    for (int i = 0 ; i < count_ring5 ; i++)
    {
        for (int j = 0 ; j < ring5[i].size() ; j++)
        {
            for (int k = 0 ; k < count_ring6 ; k++)
            {
                for (int l = 0 ; l < ring6[k].size() ; l++)
                {
                    if (j+1 < ring5[i].size() && ( (ring5[i][j] == ring6[k][l] && ring5[i][j+1] == ring6[k][l+1]) || (ring5[i][j] == ring6[k][l+1] && ring5[i][j+1] == ring6[k][l]) )  )
                    {
                        counter++;
                        neigh_ring_counter++;
                        temp_vec4[0] = i+1;
                        ...
                        ring5_ring6_neigh.push_back(temp_vec4);
        ...
        N_ring5_ring6_neigh.push_back(neigh_ring_counter);
        neigh_ring_counter = 0;
        
        sort(temp_vecn.begin(), temp_vecn.end());
        temp_vecn.erase(unique(temp_vecn.begin(), temp_vecn.end()), temp_vecn.end());
        My_neigh_ring5_ring6.push_back(temp_vecn);
        temp_vecn.clear();
        
    }
    
    N_ring5_ring6_neigh.clear();
    for (int i = 0 ; i < count_ring5 ; i++)
    {
        N_ring5_ring6_neigh.push_back(My_neigh_ring5_ring6[i].size()) ;
    }
...
void find_shared_edges_ring4_ring5(int count_ring4, int count_ring5, vector<vector<int>>& ring4, vector<vector<int>>& ring5, vector<vector<int>>& My_neigh_ring4_ring5, vector<unsigned long int>& N_ring4_ring5_neigh ){
    ...
    int counter=0;
    int neigh_ring_counter=0;
    //vector<int> N_ring4_ring5_neigh;
    vector<vector<int>> ring4_ring5_neigh;
    vector<int> temp_vec4 = {0,0,0,0};


    
    
    for (int ring4_idx = 0 ; ring4_idx < count_ring4 ; ring4_idx++)
    {
        for (int ring4_node = 0 ; ring4_node < ring4[ring4_idx].size() ; ring4_node++) //0,1,2,3,4
        {
            for (int ring5_idx = 0 ; ring5_idx < count_ring5 ; ring5_idx++)
            {
                for (int ring5_node = 0 ; ring5_node < ring5[ring5_idx].size() ; ring5_node++) //vertices
                {

                    // why not just loop over size()-1 with ring4_node instead of this?? ring4_idx understand that the last index is unnecessary because it is just a1 repeated so??
                    if (ring4_node+1 < ring4[ring4_idx].size() && ( (ring4[ring4_idx][ring4_node] == ring5[ring5_idx][ring5_node] && ring4[ring4_idx][ring4_node+1] == ring5[ring5_idx][ring5_node+1]) || (ring4[ring4_idx][ring4_node] == ring5[ring5_idx][ring5_node+1] && ring4[ring4_idx][ring4_node+1] == ring5[ring5_idx][ring5_node]) )  )
                    {
                        counter++;
                        neigh_ring_counter++;
                        temp_vec4[0] = ring4_idx+1;
                        temp_vec4[1] = ring5_idx+1;
                        temp_vec4[2] = ring4[ring4_idx][ring4_node];
                        temp_vec4[3] = ring4[ring4_idx][ring4_node+1];
                        
                        ring4_ring5_neigh.push_back(temp_vec4);
        ...
        N_ring4_ring5_neigh.push_back(neigh_ring_counter);
        neigh_ring_counter = 0;
        
        sort(temp_vecn.begin(), temp_vecn.end());
        temp_vecn.erase(unique(temp_vecn.begin(), temp_vecn.end()), temp_vecn.end());
        My_neigh_ring4_ring5.push_back(temp_vecn);
        temp_vecn.clear();
        
    }
    
    N_ring4_ring5_neigh.clear();
    for (int ring4_idx = 0 ; ring4_idx < count_ring4 ; ring4_idx++)
    {
        N_ring4_ring5_neigh.push_back(My_neigh_ring4_ring5[ring4_idx].size()) ;
    }
...
void find_shared_edges_ring6_ring4(int count_ring6, int count_ring4, vector<vector<int>>& ring6, vector<vector<int>>& ring4, vector<vector<int>>& My_neigh_ring6_ring4, vector<unsigned long int>& N_ring6_ring4_neigh ){
    ...
    int counter=0;
    int neigh_ring_counter=0;
    //vector<int> N_ring6_ring4_neigh;
    vector<vector<int>> ring6_ring4_neigh;
    vector<int> temp_vec4 = {0,0,0,0};
    ...
    for (int i = 0 ; i < count_ring6 ; i++)
    {
        for (int j = 0 ; j < ring6[i].size() ; j++)
        {
            for (int k = 0 ; k < count_ring4 ; k++)
            {
                for (int l = 0 ; l < ring4[k].size() ; l++)
                {
                    if (j+1 < ring6[i].size() && ( (ring6[i][j] == ring4[k][l] && ring6[i][j+1] == ring4[k][l+1]) || (ring6[i][j] == ring4[k][l+1] && ring6[i][j+1] == ring4[k][l]) )  )
                    {
                        counter++;
                        neigh_ring_counter++;
                        temp_vec4[0] = i+1;
                        ...
                        ring6_ring4_neigh.push_back(temp_vec4);
        ...
        N_ring6_ring4_neigh.push_back(neigh_ring_counter);
        neigh_ring_counter = 0;
        
        sort(temp_vecn.begin(), temp_vecn.end());
        temp_vecn.erase(unique(temp_vecn.begin(), temp_vecn.end()), temp_vecn.end());
        My_neigh_ring6_ring4.push_back(temp_vecn);
        temp_vecn.clear();
        
    }
    
    N_ring6_ring4_neigh.clear();
    for (int i = 0 ; i < count_ring6 ; i++)
    {
        N_ring6_ring4_neigh.push_back(My_neigh_ring6_ring4[i].size()) ;
    }
...
int remove_duplicates_map(vector<vector<Ring>>& cups)
{
    ...
    vector<pair<int, int>> current;                                        //Cup which is being read and compared at each loop.
    ...
    {
        // std::cout<<"\n"<<endl;
        for(int j = 1 ; j < cup_size ; j++){
            // std::cout<<cups[i][j].index<<endl;
            // std::cout<<"type"<<cups[i][j].type<<endl;
            current.push_back({cups[i][j].index, cups[i][j].type});
        }      //Loop to put the read cup into "current".
        sort(current.begin(), current.end());

        int duplicate_count= 0;
        for(int j = 0 ; j < current.size()-1 ; j++ )        //current.size()-1 because we compare element j to j+1
        {
            if(current[j].first == current[j+1].first && current[j].second == current[j + 1].second) {
                duplicate_count++;
            }
            // std::cout<<"duplicate"<<duplicate_count<<endl;
        }
        
        for (const auto& p : current) {
                index_key.push_back(p.first);
            }

        if(duplicate_count == 0)
        {

            if(!mymap[index_key])                  //For each read cup, check if it exists in the map. If it does NOT, write it in the map.
                
            {
                // std:cout<<"no duplciates" <<endl;
            ...
            else {mymap[index_key]++;}        //If the cup already exists in the map, jump the writing step and add one to frequencty of that cup.
        }
...
int remove_duplicates_map_cage_rings(vector<vector<int>>& cups) // TODO this was just copied for now, because int doesnt work if its cups and Ring doesnt work if its cage_..._rings
{
    unsigned long int cup_size;
    if(cups.size() != 0){cup_size = cups[0].size();}         //Number of rings in each cup, including base ring.
    else cup_size = 0;
...
int remove_duplicates_map_rings(vector<vector<int>>& rings)
{
    unsigned long int ring_size;
    if(rings.size() != 0){ring_size = rings[0].size();}         //Number of rings in each cup, including base ring.
    else ring_size = 0;
    
    unsigned long int N_rings = rings.size();                   //Number of rings before removing the duplicates.
    int count=0;                                                //Number of rings.
    vector<int> current;                                        //ring which is being read and compared at each loop.
    map<vector<int>,int> mymap;                                 //A map which counts the frequencty of each ring( [ring,frequency] )
    int prev = 0 ;
    
    for (int i = 0 ; i < N_rings ; i++)      //Loop over all the rings.
    {
        for(int j = 1 ; j < ring_size ; j++){
            current.push_back(rings[i][j]);
        }      //Loop to put the read ring into "current".
        sort(current.begin(), current.end());
        // for(int j = 0 ; j < current.size() ; j++){
        //     std::cout<<current[j]<<endl;
        // }      //Loop to put the read ring into "current".
        int duplicate_count= 0;
        for(int j = 0 ; j < current.size() ; j++ )                
        {
        ...
        // std::cout<<"duplicate count"<<duplicate_count<<endl;
        if(duplicate_count <= 1)
        {
            if(!mymap[current])                  //For each read ring, check if it exists in the map. If it does NOT, write it in the map.
                
            {
                ...
                {
                    rings[prev][k] = rings[i][k];   //If the ring does not exist in map, write it to first line of original ring variable.
                }
                prev++;
            }
            else {mymap[current]++;}        //If the ring already exists in the map, jump the writing step and add one to frequencty of that cup.
        }
        else duplicate_count = 0;
...
#include <math.h>
    ...
    for (int r = 0 ; r < N_rings ; r++)
    {
        ...
        if(N_vertices == 5)
        {
            if( theta1 < THETA)      //If a ring is planar, do these:
            {
        ...
        else if(N_vertices == 6)     //replacing original ring5 with planar rings.
        {
...
void cup_512_Finder(vector<vector<int>>& ring5,int count_ring5, vector<unsigned long int>& N_ring5_neigh, vector<vector<int>>& My_neigh_ring5, vector<vector<Ring>>& cup512 ){
    // TODO the naming scheme here doesnt fit with the rest at all
                                                                                    ...
                                                                                    if (repeated_atoms.size() <= 2)
                                                                                    {
                                                                                        ...
                                                                                        if (repeated_atoms.size() <= 2)
                                                                                        {
                                                                                            ...
                                                                                            if (repeated_atoms.size() <= 2)
                                                                                            {
                                                                                                ...
                                                                                                Ring r1 = {first_ring, PENTAGON};
                                                                                                Ring r2 = {second_ring, PENTAGON};
                                                                                                Ring r3 = {third_ring, PENTAGON};
                                                                                                Ring r4 = {fourth_ring, PENTAGON};
                                                                                                Ring r5 = {fifth_ring, PENTAGON};
                                                                                                Ring r6 = {sixth_ring, PENTAGON};
                                                                                                
                                                                                                cup512.push_back({r1, r2, r3, r4, r5, r6});
                                                                                                                ...
                                                                                                                if (repeated_atoms.size() <= 2)
                                                                                                                {
                                                                                                                    ...
                                                                                                                    if (repeated_atoms.size() <= 2)
                                                                                                                    {
                                                                                                                        ...
                                                                                                                        if (repeated_atoms.size() <= 2)
                                                                                                                        {
                                                                                                                            ...
                                                                                                                            Ring r1 = {first_ring, HEXAGON};
                                                                                                                            Ring r2 = {second_ring, PENTAGON};
                                                                                                                            Ring r3 = {third_ring, PENTAGON};
                                                                                                                            Ring r4 = {fourth_ring, PENTAGON};
                                                                                                                            Ring r5 = {fifth_ring, PENTAGON};
                                                                                                                            Ring r6 = {sixth_ring, PENTAGON};
                                                                                                                            Ring r7 = {seventh_ring, PENTAGON};
                                                                                                                            
                                                                                                                            cup62512.push_back({r1, r2, r3, r4, r5, r6, r7});
...
void cup_415462_Finder(vector<vector<int>>& ring6, int count_ring6, vector<vector<int>>& My_neigh_ring6_ring5, vector<vector<int>>& My_neigh_ring5_ring6, vector<vector<int>>& My_neigh_ring6_ring4,vector<vector<int>>& My_neigh_ring4_ring5, vector<unsigned long int> N_ring6_ring5_neigh, vector<unsigned long int> N_ring5_ring6_neigh, vector<unsigned long int> N_ring6_ring4_neigh, vector<unsigned long int> N_ring4_ring5_neigh, vector<vector<int>>& ring5, vector<unsigned long int> N_ring5_neigh, vector<vector<int>>& My_neigh_ring5, vector<vector<int>>& ring4, int count_ring4, vector<vector<int>>& My_neigh_ring4, vector<vector<Ring>>& cup415462){
    ...
    // finding half of 4(1)5(10)6 cages

    int first_ring_square, second_ring_pentagon, third_ring_hexagon, fourth_ring_pentagon, fifth_ring_hexagon, sixth_ring_pentagon, seventh_ring_pentagon;
    vector<int> temp_ring1, temp_ring2, temp_ring3, temp_ring4, temp_ring5, temp_ring6, temp_ring7;
    vector<int> repeated_atoms, temp_vec6={0,0,0,0,0,0,0};

    for (int index_s1 = 0 ; index_s1 < count_ring4 ; index_s1++)
    {
        first_ring_square = index_s1;

        if (N_ring4_ring5_neigh[first_ring_square] >= 2)       // check if there are at least 2 pentagonal neighbours
        {
            for (int index2 = 0 ; index2 < N_ring4_ring5_neigh[first_ring_square] ; index2++)  // loop over all square neighbours of the hexagon
            {
                second_ring_pentagon = My_neigh_ring4_ring5[first_ring_square][index2];
                if (N_ring5_ring6_neigh[second_ring_pentagon] >= 2)
                {
                    for (int index3 = 0 ; index3 < N_ring5_ring6_neigh[second_ring_pentagon] ; index3++)
                    {
                        // std::cout<< "third hexagon indices: " << index

                        // if(compare_adjacant(ring5[second_ring_pentagon], ring5[My_neigh_ring5_ring6[second_ring_pentagon][index3]], 5, 5, ring6[first_ring_square]) )
                        // {
                        
                        third_ring_hexagon = My_neigh_ring5_ring6[second_ring_pentagon][index3];
                        if (N_ring6_ring4_neigh[third_ring_hexagon] >= 1 &&
                            N_ring6_ring5_neigh[third_ring_hexagon] >= 3)
                        {

                            for (int index4 = 0 ; index4 < N_ring6_ring5_neigh[third_ring_hexagon]; index4++)
                            {
                    
                                if (
                                    compare(ring4[first_ring_square], ring5[My_neigh_ring6_ring5[third_ring_hexagon][index4]], 4, 5)
                                    )
                                {
                                    
                                    
                                    fourth_ring_pentagon = My_neigh_ring6_ring5[third_ring_hexagon][index4];
                                    if (N_ring5_ring6_neigh[fourth_ring_pentagon] >= 2 && fourth_ring_pentagon != second_ring_pentagon)
                                    {
                                    
                                        for (int index5 = 0 ; index5 < N_ring5_ring6_neigh[fourth_ring_pentagon]; index5++)
                                        {
                                            if (My_neigh_ring5_ring6[fourth_ring_pentagon][index5] != third_ring_hexagon &&
                                                compare(ring4[first_ring_square], ring6[My_neigh_ring5_ring6[fourth_ring_pentagon][index5]], 4, 6) &&
                                                compare(ring5[second_ring_pentagon], ring6[My_neigh_ring5_ring6[fourth_ring_pentagon][index5]], 5, 6)
                                                //compare_adjacant(ring5[fourth_ring_pentagon], ring5[My_neigh_ring5[fourth_ring_pentagon][index5]], 5, 5, ring6[first_ring_square] )
                                                )
                                            {
                                                fifth_ring_hexagon = My_neigh_ring5_ring6[fourth_ring_pentagon][index5];
                                                if (N_ring6_ring5_neigh[fifth_ring_hexagon] >= 3)
                                                {
                                                    // std::cout<< "S "<< first_ring_square << " P " << second_ring_pentagon << " H " << third_ring_hexagon << " P " << fourth_ring_pentagon << " H " << fifth_ring_hexagon <<endl;

                                                    for (int index6 = 0 ; index6 < N_ring6_ring5_neigh[fifth_ring_hexagon]; index6++)
                                                    {
                                                        // std::cout<< "sixth ring pentagon possibility: " << My_neigh_ring6_ring5[fifth_ring_hexagon][index6] << endl;
                                                        if (My_neigh_ring6_ring5[fifth_ring_hexagon][index6] != second_ring_pentagon &&
                                                            My_neigh_ring6_ring5[fifth_ring_hexagon][index6] != fourth_ring_pentagon &&
                                                            compare(ring5[second_ring_pentagon], ring5[My_neigh_ring6_ring5[fifth_ring_hexagon][index6]], 5, 5) //&&
                                                            // compare_adjacant(ring5[fifth_ring_hexagon], ring5[My_neigh_ring5[fifth_ring_hexagon][index6]], 5, 5, ring6[first_ring_square])
                                                            )
                                                            
                                                        {
                                                            // std::cout<< "sixth ring pentagon possibility after first if: " << My_neigh_ring6_ring5[fifth_ring_hexagon][index6] << endl;
                                                            sixth_ring_pentagon = My_neigh_ring6_ring5[fifth_ring_hexagon][index6];


                                                            // std::cout<< "sixth ring pentagon possibility: " << sixth_ring_pentagon << endl;

                                                            
                                                            
                                                            if (N_ring5_neigh[sixth_ring_pentagon] >= 2 &&
                                                                N_ring5_ring6_neigh[sixth_ring_pentagon] >= 1 )
                                                            {
                                                            
                                                                
                                                                for (int index7 = 0 ; index7 < N_ring5_neigh[sixth_ring_pentagon]; index7++)
                                                                {
                                                            
                                                                    if (My_neigh_ring5[sixth_ring_pentagon][index7] != second_ring_pentagon &&
                                                                        My_neigh_ring5[sixth_ring_pentagon][index7] != fourth_ring_pentagon &&
                                                                        My_neigh_ring5[sixth_ring_pentagon][index7] != sixth_ring_pentagon  &&
                                                                        compare(ring5[second_ring_pentagon], ring5[My_neigh_ring5[sixth_ring_pentagon][index7]], 6, 5) &&
                                                                        compare(ring6[third_ring_hexagon], ring5[My_neigh_ring5[sixth_ring_pentagon][index7]], 6, 5) //&&
                                                                        // compare_adjacant(ring5[sixth_ring_pentagon], ring5[My_neigh_ring5[sixth_ring_pentagon][index7]], 5, 5, ring6[first_ring_square])
                                                                        )
    
                                                                    {
                                                                        
                                                                        seventh_ring_pentagon = My_neigh_ring5[sixth_ring_pentagon][index7];

                                                                        // std::cout<< "S "<< first_ring_square << " P " << second_ring_pentagon << " H " << third_ring_hexagon << " P " << fourth_ring_pentagon << " H " << fifth_ring_hexagon << " P " << sixth_ring_pentagon << " P " << seventh_ring_pentagon << endl;

                                                                        for (int index8 = 0 ; index8 < N_ring6_ring5_neigh[third_ring_hexagon]; index8++)
                                                                        {
                                                                            
                                                                            if(My_neigh_ring6_ring5[third_ring_hexagon][index8] == seventh_ring_pentagon)
                                                                            {
                                                                                // std::cout<< "S "<< first_ring_square << " P " << second_ring_pentagon << " H " << third_ring_hexagon << " P " << fourth_ring_pentagon << " H " << fifth_ring_hexagon << " P " << sixth_ring_pentagon << " P " << seventh_ring_pentagon << endl;

                                                                                
                                                                                if (compare(ring6[third_ring_hexagon], ring5[seventh_ring_pentagon], 6, 5))
                                                                                {
                                                                                    // std::cout<< "S "<< first_ring_square << " P " << second_ring_pentagon << " H " << third_ring_hexagon << " P " << fourth_ring_pentagon << " H " << fifth_ring_hexagon << " P " << sixth_ring_pentagon << " P " << seventh_ring_pentagon << endl;
                                                                                    ...
                                                                                    for (int i_1 = 0; i_1 < 6; i_1++)
                                                                                    {
                                                                                        ...
                                                                                        temp_ring3.push_back(ring6[third_ring_hexagon ][i_1]);
                                                                                        temp_ring5.push_back(ring6[fifth_ring_hexagon ][i_1]);
                                                                                        
                                                                                    }

                                                                                    for (int i_1 = 0; i_1 < 5; i_1++)
                                                                                    {
                                                                                        temp_ring2.push_back(ring5[second_ring_pentagon][i_1]);
                                                                                        temp_ring4.push_back(ring5[fourth_ring_pentagon][i_1]);
                                                                                        temp_ring6.push_back(ring5[sixth_ring_pentagon][i_1]);
                                                                                        temp_ring7.push_back(ring5[seventh_ring_pentagon][i_1]);
                                                                                    }
                                                                                    for (int i_1 = 0; i_1 < 4; i_1++)
                                                                                    {
                                                                                        temp_ring1.push_back(ring4[first_ring_square ][i_1]);
                                                                                    }
                                                                                    ...
                                                                                    if (repeated_atoms.size() <= 2)
                                                                                    {
                                                                                        ...
                                                                                        if (repeated_atoms.size() <= 2)
                                                                                        {
                                                                                            ...
                                                                                            if (repeated_atoms.size() <= 2)
                                                                                            {
                                                                                                ...
                                                                                                repeated_atoms.clear();
                                                                                                set_intersection(temp_ring4.begin(),temp_ring4.end(),temp_ring5.begin(),temp_ring5.end(),back_inserter(repeated_atoms));            //TODO why is set intersection not used for the remove duplicates
                                                                                                // for (int i =0; i<repeated_atoms.size();i++) {
                                                                                                //     std::cout<<repeated_atoms[i]/4<<endl;
                                                                                                // }
                                                                                                if (repeated_atoms.size() <= 2)
                                                                                                {
                                                                                                    repeated_atoms.clear();
                                                                                                    ...
                                                                                                    if (repeated_atoms.size() <= 2)
                                                                                                    {
                                                                                                        ...
                                                                                                        if (repeated_atoms.size() <= 2)
                                                                                                        {
                                                                                                            ...
                                                                                                            if (repeated_atoms.size() <= 2)
                                                                                                            {
                                                                                                                ...
                                                                                                                Ring r1 = {first_ring_square , SQUARE};
                                                                                                                Ring r2 = {second_ring_pentagon , PENTAGON};
                                                                                                                Ring r3 = {third_ring_hexagon, HEXAGON};
                                                                                                                Ring r4 = {fourth_ring_pentagon , PENTAGON};
                                                                                                                Ring r5 = {fifth_ring_hexagon, HEXAGON};
                                                                                                                Ring r6 = {sixth_ring_pentagon, PENTAGON};
                                                                                                                Ring r7 = {seventh_ring_pentagon, PENTAGON};
                                                                                                                
                                                                                                                cup415462.push_back({r1, r2, r3, r4, r5, r6, r7});
...
int cage_Finder(std::string cagetype, vector<vector<Ring>> cups, unsigned long count_cups, vector<vector<int>>& My_neigh_ring5, vector<vector<int>>& cage, vector<vector<int>>& cage_rings, string time )
{ //TODO this is not at all general because it only looks for pentagon side rings 
    
    int count = 0;      //Counter for number of cages.
    vector<Ring> current_cup;
    int current_ring = 0 ;
    int N_core_rings = 0;
    ...
    if (cagetype == "cage512" || cagetype == "cage62512") {
        N_core_rings = 1;
    } 
    else if (cagetype == "cage4151062") {
        N_core_rings = 2;
    }


    for (int cup_1_idx = 0  ; cup_1_idx < count_cups ; cup_1_idx++) {     //Loop over all the cups.
        current_cup = cups[cup_1_idx]; // Directly use the whole cup as a vector of Ring structs.

        for (int cup_2_idx = cup_1_idx+1 ; cup_2_idx < count_cups ; cup_2_idx++) {       //Loop over the rest of the cups to compare with current cup
            for(int side_ring_1_idx = N_core_rings ; side_ring_1_idx < current_cup.size() ; side_ring_1_idx++) {                           //Loop over side rings of current cup.
                                                                                    //"side_ring_1_idx" is the counter for side rings of current cup. It starts from 1
                                                                                    //because first ring in each cup is the base ring.
                current_ring = current_cup[side_ring_1_idx].index;
                for(int side_ring_2_idx = N_core_rings ; side_ring_2_idx < cups[cup_2_idx].size() ; side_ring_2_idx++) {                    //Loop over side rings of the rest of the cups.

                    for(int neighbor_idx = 0 ; neighbor_idx < My_neigh_ring5[current_ring].size() ; neighbor_idx++) {     //Loop over neighbour rings of current ring.
                                                                                        //"neighbor_idx" is counter for all the neighbour rings of
                                                                                        //side rings of current cup.
                        if( My_neigh_ring5[current_ring][neighbor_idx] == cups[cup_2_idx][side_ring_2_idx].index ) {
                            N++;
                        }
                    }
                }
                if(N==2) { 
                    M++; 
                    N = 0;
                } else {                    //If current side ring has 2 neighbors in other cups, move on
                                            //to next side ring of the current cup.
                    N = 0; 
                    break;
                }                           //If current side ring has less than 2 neighbors, move on to the next cup.
            }

            if(M == cup_size - 1) {
                
                for(int h = 0 ; h < cup_size  ; h++) {
                    temp_vec_rings.push_back(cups[cup_1_idx][h].index);
                }
                for(int h = 0 ; h < cup_size  ; h++) {
                    temp_vec_rings.push_back(cups[cup_2_idx][h].index);
                }
                cage_rings.push_back(temp_vec_rings);
                temp_vec_rings.clear();
                
                temp_vec.push_back(cup_1_idx);
                temp_vec.push_back(cup_2_idx);
...
int cage_Finder_different_cups(std::string cagetype, vector<vector<Ring>> cups1, unsigned long count_cups1, vector<vector<Ring>> cups2, unsigned long count_cups2, vector<vector<int>> ring5,  vector<vector<int>>& My_neigh_ring5,  vector<vector<int>>& My_neigh_ring6, vector<vector<int>>& My_neigh_ring6_ring5, vector<vector<int>>& cage, vector<vector<Ring>>& cage_rings, string time )
{
    ...
    RingType current_ring_type;
    vector<int> neigh_idx;
    int N = 0;      //counter for number of common rings between side rings of current cup and side rings of rest of the cups.
    int M = 0;      //counter for number of side rings of current cup which have 2 common rings with side rings of rest of the cups.
    int N_core_rings_1 = 0;
    int N_core_rings_2 = 0;
    unsigned long int cup1_size=0;
    if( cups1.size() != 0 ){cup1_size = cups1[0].size();}
    vector<int> temp_vec;
    vector<Ring> temp_vec_rings;
    if (cagetype == "cage4151062") {
    N_core_rings_1 = 2;
    N_core_rings_2 = 1;
    }

    // TODO in general i dont need to add the Ring struct into the cages, but maybe make a new one for cups

    // here cup 1 is the 415462 and cup 2 56
    
    for (int cup_1_idx = 0  ; cup_1_idx < count_cups1 ; cup_1_idx++) {     //Loop over all the cups.
        current_cup = cups1[cup_1_idx]; // Directly use the whole cup as a vector of Ring structs.

        for (int cup_2_idx = 0 ; cup_2_idx < count_cups2 ; cup_2_idx++) {       //Loop over the second type of cups to compare with current cup
            // std::cout << cups2[cup_2_idx].size() << endl;
            // std::cout << cups2[cup_2_idx][3].type << endl;
            // std::cout<<"new cup"<<endl;
            for(int side_ring_1_idx = N_core_rings_1 ; side_ring_1_idx < current_cup.size() ; side_ring_1_idx++) {                           //Loop over side rings of current cup.
                                                                                    //"side_ring_1_idx" is the counter for side rings of current cup. It starts from the number of core rings                

                current_ring_idx = current_cup[side_ring_1_idx].index;
                // std::cout<<"idx: "<<current_ring_idx<<endl;
                current_ring_type = current_cup[side_ring_1_idx].type;
                // std::cout<<"type: "<<current_ring_type<<endl;


                neigh_idx.clear() ;
                if ( current_ring_type == PENTAGON ) {
                    neigh_idx = My_neigh_ring5[current_ring_idx] ;
                    // std::cout<<"PENTAGON"<<endl;
                } else if ( current_ring_type == HEXAGON ) {
                    neigh_idx = My_neigh_ring6_ring5[current_ring_idx] ;
                    // std::cout<<"HEXAGON"<<endl;
                }
                // currently only implemented pentagon and hexagon side rings because i think its never squares

                // std::cout<<"\n neighbours: "<<endl;
                // for (int k = 0; k < neigh_idx.size(); k++) {
                //     std::cout<<"neigh"<<neigh_idx[k]<<endl;
                // }

                for( int side_ring_2_idx = N_core_rings_2 ; side_ring_2_idx < cups2[cup_2_idx].size() ; side_ring_2_idx++) {                    //Loop over side rings of the rest of the cups.
                    
                    // std::cout<<"cup2 ring "<<cups2[cup_2_idx][side_ring_2_idx].index<<endl;
                    

                    for(int neighbor_idx = 0 ; neighbor_idx < neigh_idx.size() ; neighbor_idx++) {     //Loop over neighbour rings of current ring.
                                                                                        //"neighbor_idx" is counter for all the neighbour rings of
                                                                                        //side rings of current cup.
                        if( neigh_idx[neighbor_idx] == cups2[cup_2_idx][side_ring_2_idx].index ) {
                            N++;
                        }
                    }
                }
            ...
            // std::cout<<"MMMMMM"<<M<<endl;

            if(M == cup1_size - N_core_rings_1) {        // TODO check if it makes sense to compare to cup1 size (-1 is because first ring will also be saved as last ring)
                // std::cout<<"this should add a cage"<<endl;
                for(int h = 0 ; h < cup1_size  ; h++) {
                    temp_vec_rings.push_back(cups1[cup_1_idx][h]);
                }
                for(int h = 0 ; h < cup1_size  ; h++) {
                    temp_vec_rings.push_back(cups2[cup_2_idx][h]);
                }
                cage_rings.push_back(temp_vec_rings);
                temp_vec_rings.clear();
...
//     //Find methanes closeest to Center of Mass of each cage and write them to -temp.gro file output.
//     vector<int> temp_solute_number_holder;
//     temp_solute_number_holder.clear();
...
//                 if(find(temp_solute_number_holder.begin(), temp_solute_number_holder.end(), j) == temp_solute_number_holder.end()) //If the value of "j" (methane index) does not exist in the temp_solute_number_holder, then add it to the vector and write its coordinates to gro file. Otherwise, do not write it in gro output file. The find(v.begin(),v.end(),"value") gives the iterator number of found value in the vector. If the value is not in the vector, iterator will be equal to v.end().
//                 {
//                     temp_solute_number_holder.push_back(j);
...
//     unsigned long int N_cage = cage_count;        //This is the number of cages after removing the duplicates. This should be the correct number to use, however, using it will reduce the number of cages printed in gro file output. Something is not right here!
//     int atoms=0;
//     int ring_idx;       //"i" is cup number. "p" is ring number.
//     int ring_type;
//     int ring_size;
//     int atom_counter=1;        //"atom_counter" is a counter for atom number in gro file. It should reset to one for each frame.
//     int sol_counter=1;         //counter for number of SOL in each frame.
...
//                                          //This number is not neccesarily correct, e.x.,if cages share a ring with each other(That's why
//                                          //the re-writting of output file corrects it at the end).
//         out = rawFilename + "_cage512-temp.gro";
//     }
//     else if(cagetype == "cage62512"){
//         tot_atoms = N_cage * 24 * 4 ;    //each 62512 cage has 24 water molecule. TIP4P has 4 atoms per molecule.
//                                          //This number is not neccesarily correct, e.x., if cages share a ring with each other(That's why
//                                          //the re-writting of output file corrects it at the end).
//         out = rawFilename + "_cage62512-temp.gro";
//     }
//     else if(cagetype == "cage64512"){
//         tot_atoms = N_cage * 28 * 4 ;    //each 62512 cage has 24 water molecule. TIP4P has 4 atoms per molecule.
//                                          //This number is not neccesarily correct, e.x., if cages share a ring with each other(That's why
//                                          //the re-writting of output file corrects it at the end).
//         out = rawFilename + "_cage64512-temp.gro";
//     }
//     else if(cagetype == "cage4151062"){
//         tot_atoms = N_cage * 22 * 4 ;    //each 62512 cage has 24 water molecule. TIP4P has 4 atoms per molecule.
//                                          //This number is not neccesarily correct, e.x., if cages share a ring with each other(That's why
//                                          //the re-writting of output file corrects it at the end).
//         out = rawFilename + "_cage4151062-temp.gro";
//     }
...
//     for(int cage_idx = 0 ; cage_idx < N_cage ; cage_idx++)       //Loop over all the cages.
//     {
        
//         current_cage_com.clear();

//         for(int ring_idx_gen= 0 ; ring_idx_gen< N_rings; ring_idx_gen++)         //Loop over 6 or 7 rings of each cage.
//         {
//             ring_idx = cage_rings[cage_idx][ring_idx_gen].index;
//             ring_type = cage_rings[cage_idx][ring_idx_gen].type;
//             {
//                 if(ring_type == HEXAGON)  //This part is for base ring of 6(2)5(12) cages which is a 6-ring.
//                 {
//                     ring_size = 6;
//                     for (int q = 0 ; q < ring_size ; q++)       //Loop over all atoms of a ring.
//                     {
//                         atoms = ring6[ring_idx][q];
//                         if(!atoms_set.count(atoms))
//                         {                               //If atoms is not present in atoms_set, count will be zero, condition is one and
//                                                         //the rest of this block will be executed.
//                                                         //If atoms already exists in the set, it will not print out.
...
//                     }
//                 }
//                 else if(ring_type == PENTAGON)           //This part is for all the rings of 5(12) cages which are all 5-rings.
//                 {
//                     ring_size = 5;
//                     for (int q = 0 ; q < ring_size ; q++)       //Loop over all atoms of a ring.
//                     {
//                         atoms = ring5[ring_idx][q];
//                         if(!atoms_set.count(atoms))
//                         {                               //If atoms is not present in atoms_set, count will be zero, condition is one and
//                                                         //the rest of this block will be executed.
//                                                         //If atoms already exists in the set, it will not print out.
...
//                 }
//                 else if(ring_type == SQUARE)           //This part is for all the rings of 5(12) cages which are all 5-rings.
//                 {
//                     ring_size = 4;
//                     for (int q = 0 ; q < ring_size ; q++)       //Loop over all atoms of a ring.
//                     {
//                         atoms = ring4[ring_idx][q];
//                         if(!atoms_set.count(atoms))
//                         {                               //If atoms is not present in atoms_set, count will be zero, condition is one and
//                                                         //the rest of this block will be executed.
//                                                         //If atoms already exists in the set, it will not print out.
...
//     {
//         current_cage_com.clear();
//         for(int ring_idx_gen = 0 ; ring_idx_gen < N_rings; ring_idx_gen++)         //Loop over all rings of each cage.
//         {
//             ring_idx = cage_rings[cage_idx][ring_idx_gen].index;
//             ring_type = cage_rings[cage_idx][ring_idx_gen].type;
    ...
    for (int i = 0 ; i < count_62512_cups - 1 ; i++)
    {
        ...
        for (int j = i+1 ; j < count_62512_cups ; j++)
        {
            for (int k = 1 ; k < 7 ; k++)
            {
                for (int l = 1 ; l < 7 ; l++)
                {
                    if (cup62512[i][k].index == cup62512[j][l].index)
                    {
                        N++;
...
//    }
    ...
    for (int i = 0 ; i < count_cage_64512 ; i++)
    {
        for (int j = 0 ; j < 4 ; j++ )
        {
            ...
            temp.push_back(cup62512[p][0].index);     //here temp holds the index of base rings (hexagons) of each of the four, 62512 cups.
        }
        for (int j = 0 ; j < 4 ; j++)           //this loops gets the pentagons of the four cups of cage_64512 into cage_64512_rings.
        {
            p = cage_64512[i][j];
            for(int k=1;k<=6;k++){temp.push_back(cup62512[p][k].index);}
        }
...
void print_vmd_cage_frings(std::string cagetype, int cage_count, vector<vector<Ring>> cage_rings, vector<vector<int>> ring4, vector<vector<int>> ring5, vector<vector<int>> ring6, vector<vector<double>> atom_Pos, string time, string rawFilename , string box_size_xyz, vector<vector<double>> solutes, size_t & meth_counter, string solute1, int topSolute, string solute2, int count_solute2, int frameCounter)
// TODO this seems to be the only print function being used? -> get rid of others and rename, is number of rings in cage bijective (in the cases we need)
{
    
    unsigned long int N_rings = 0;      //Number of rings in a cage. cage 512 has 12, cage 62512 has 14 and cage 64512 has 28 (because of the way the cage is built by 4 6156 cups)
    if(cage_rings.size() != 0){N_rings = cage_rings[0].size();}
    
    unsigned long int N_cage = cage_count;        //This is the number of cages after removing the duplicates. This should be the correct number to use, however, using it will reduce the number of cages printed in gro file output. Something is not right here!
    int atoms=0;
    int ring_idx;       //"i" is cup number. "ring_idx" is ring number.
    int ring_type;
    int ring_size;
    int atom_counter=1;        //"atom_counter" is a counter for atom number in gro file. It should reset to one for each frame.
    int sol_counter=1;         //counter for number of SOL in each frame.
    ...
    std::cout<<"cagecout"<<N_cage<<endl;
    std::cout << "box_size_xyz: " << box_size_xyz << std::endl;

    istringstream streamB(box_size_xyz);
    streamB >> boxX >> boxY >> boxZ ;
    
    // Use cagetype input to set the appropriate values
    if (cagetype == "cage64512") {
        tot_atoms = N_cage * 28 * 4; // Temporary value, will be corrected later
        out = out64512;
    }
    else if (cagetype == "cage62512") {
        tot_atoms = N_cage * 24 * 4;  // Each 62512 cage has 24 water molecules (TIP4P has 4 atoms per molecule)
        out = out6;
    }
    else if (cagetype == "cage512") {
        tot_atoms = N_cage * 20 * 4;  // Each 512 cage has 20 water molecules (TIP4P has 4 atoms per molecule)
        out = out5;
    }
    else if (cagetype == "cage4151062") {
        tot_atoms = N_cage * 22 * 4;  // Each 512 cage has 20 water molecules (TIP4P has 4 atoms per molecule)
        out = out5;
    }
    else {
        cerr << "Error: Invalid cage type!" << endl;
        return;
    }
    
    std::ofstream outFile (out, std::ofstream::app);     //Open file and append to it.
    outFile.setf(ios::fixed, ios::floatfield);           //Make output numbers look neat
    outFile << "Generated using GRADE : t= " << time << "\n";      //First line of gro file, comment and frame time.
    outFile << tot_atoms << "\n";
    ...
    for(int cage_idx = 0 ; cage_idx < N_cage ; cage_idx++)       //Loop over all the cages.
    {
        ...
        for(int ring_idx_gen = 0 ; ring_idx_gen < N_rings; ring_idx_gen++)         //Loop over 6 or 7 rings of each cage.
        {
            ...
            ring_type = cage_rings[cage_idx][ring_idx_gen].type;
            {
                if(ring_type == HEXAGON)   //This part is for base ring of 6(2)5(12) cups which are 6-ring are indexed 0,1,2,3.
                {
                    ring_size = 6;
                    for (int q = 0 ; q < ring_size ; q++)       //Loop over all atoms of a ring.
                    {
                ...
                else if(ring_type == PENTAGON)           //This part is for all the rings of 5(12) or lateral rings in 62512 or 64512 cages which are all 5-rings.
                {
                    ring_size = 5;
                    for (int q = 0 ; q < ring_size ; q++)       //Loop over all atoms of a ring.
                    {
                ...
                else if(ring_type == SQUARE)           //This part is for all the rings of 5(12) or lateral rings in 62512 or 64512 cages which are all 5-rings.
                {
                    ring_size = 4;
                    for (int q = 0 ; q < ring_size ; q++)       //Loop over all atoms of a ring.
                    {
    ...
    {
        current_cage_com.clear();
        for(int ring_idx_gen = 0 ; ring_idx_gen < N_rings; ring_idx_gen++)         //Loop over all rings of each cage.(12 or 14 or 28)
        {
            ...
            {
                if(ring_type == HEXAGON) //This part is for base ring of 6(2)5(12) or 6(4)5(12) cages which is a 6-ring.
                {
                    ring_size = 6;
                    for (int q = 0 ; q < ring_size ; q++)       //Loop over all atoms of a ring.
                    {
                ...
                else if( (N_rings == 14 && ring_idx_gen == 0) || (N_rings == 14 && ring_idx_gen == 7) ) //This part is for base ring of 6(2)5(12) cages which is a 6-ring.
                {
                    ring_size = 6;
                    for (int q = 0 ; q < ring_size ; q++)       //Loop over all atoms of a ring.
                    {
...
void print_data(std::ofstream& frameDataFile, int frameCounter, int cage_512_count, int cage_62512_count, 
                int cage_64512_count, double F4_value) {
    // Write the frame data to the output file in a column format
    frameDataFile << std::left << std::setw(10) << frameCounter 
                  << std::setw(15) << cage_512_count 
                  << std::setw(15) << cage_62512_count 
                  << std::setw(15) << cage_64512_count 
                  << std::setw(10) << F4_value 
                  << "\n";
}
